...
Collector::Collector(Holder& holder,int timeOutMs, int separationTimeMs, int packetSize) :
    m_module(holder),
...
void Collector::SendFrame(){
    ...
    if(m_packetSize == 3){
        m_module.receiveDetectCone(m_currentFrame);
    }
    else if(m_packetSize == 2){
        m_module.receiveAttention(m_currentFrame);
    }
}
